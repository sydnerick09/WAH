...
function PaymentModal({ task, user, onClose, onSuccess }) {
  ...
  async function handlePay() {
    ...
    try {
      ...
      if (data.status) {
        // Store intent so callback page can activate the user
        localStorage.setItem('pendingActivation', JSON.stringify({ userId: user.id, type: 'activation' }));
...
function UpgradeModal({ user, onClose, onUpgradeSuccess }) {
  const [phone, setPhone] = useState(user?.phone || '');
  const [loading, setLoading] = useState(false);
  ...
  async function handleUpgrade() {
    ...
    try {
      const res = await fetch('/api/paystack/initialize', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: user.email, amount: 130, phone, plan: 'premium' }),
      });
      const data = await res.json();
      if (data.status) {
        // Store intent so callback page can set user as active + premium
        localStorage.setItem('pendingActivation', JSON.stringify({ userId: user.id, type: 'premium' }));
        window.location.href = data.data.authorization_url;
      } else {
      ...
      <div className="pay-modal-card" style={{ maxWidth: 480 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="pay-modal-body">
          ...
          <div className="pay-amount" style={{ marginTop: 20 }}>
            <div className="pay-amount-label">Weekly Premium Plan</div>
            <div className="pay-amount-value" style={{ background: 'linear-gradient(135deg,#0047FF,#7C3AED)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>KES 130</div>
            <div className="pay-amount-sub">per week · Cancel anytime</div>
          </div>
          <div className="pay-phone-label">M-Pesa / Mobile Money Number</div>
          <input
            className="pay-phone-input"
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? <><span className="spinner" /> Processing...</> : '⭐ Upgrade to Premium — KES 130/week'}
          </button>
          <div className="pay-secure">🔐 Secured by Paystack · M-Pesa supported</div>
        </div>
      </div>
      ...
      <div className="pay-modal-card" style={{ maxWidth: 400 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="pay-modal-body" style={{ textAlign: 'center', padding: '36px 28px' }}>
          ...
          <button
            ...
            onClick={onUpgrade}
          >
            ⭐ Upgrade to Premium — KES 130/week
          </button>
      ...
      <div className="pay-modal-card" style={{ maxWidth: 460 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="pay-modal-body">
          ...
          <div className="referral-stats">
            ...
            <div className="referral-stat">
              <div className="referral-stat-num">KES 0</div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
            {[
              { label: '📱 WhatsApp', color: '#25D366', url: `https://wa.me/?text=Join%20Online%20Job%20and%20earn%20online!%20${encodeURIComponent(referralLink)}` },
              { label: '✉️ Email', color: '#EA4335', url: `mailto:?subject=Join%20Online%20Job&body=Hey!%20Join%20me%20on%20Online%20Job%20and%20start%20earning%20online.%20Use%20my%20link:%20${referralLink}` },
            ].map(btn => (
              <a
                key={btn.label}
                href={btn.url}
...
function HamburgerMenu({ user, onClose, onUpgrade, onWithdraw, onReferral, onLogout }) {
  const items = [
    ...
    { icon: '🎓', label: 'Apply for Training', action: () => { onClose(); window.location.href = 'mailto:businesshub@gmail.com?subject=Training Application&body=Hello, I would like to apply for training. My name is ' + (user?.fullName || '') + ' and my email is ' + (user?.email || '') + '.'; } },
...
export default function Dashboard() {
  ...
  // ── Check for successful Paystack callback and activate user ──────────────
  useEffect(() => {
    if (!mounted || !user) return;

    const { query } = router;
    // Paystack redirects back with ?trxref=...&reference=... on success
    const reference = query.reference || query.trxref;

    if (reference) {
      const pending = (() => {
        try { return JSON.parse(localStorage.getItem('pendingActivation') || 'null'); } catch { return null; }
      })();
        ...
        fetch(`/api/paystack/verify?reference=${reference}`)
          ...
          .then(data => {
            if (data.status && data.data?.status === 'success') {
              // Activate user in your auth store
              const updated = activateUser(user.id);
              if (updated) {
                setUser({ ...updated, activated: true });
              }
  ...
  const handlePaySuccess = useCallback(() => {
    const updated = activateUser(user.id);
    if (updated) setUser({ ...updated, activated: true });
  }, [user]);
    ...
    <div className="dashboard">
      ...
      <nav className="dash-navbar">
        <div className="dash-navbar-inner">
          <Link href="/" className="dash-logo">ONLINE JOB</Link>
      ...
      <main className="dash-main">
        ...
        <div className="quick-actions">
          ...
          <a
            href={`mailto:businesshub@gmail.com?subject=Training Application&body=Hello, I would like to apply for training. My name is ${user.fullName} and my email is ${user.email}.`}
          ...
          <div className="tasks-grid">
            {filteredTasks.map(task => (
              <div key={task.id} className="task-card">
                ...
                <button className="task-view-btn" onClick={() => setSelectedTask(task)}>
                  👁️ View / Bid
                </button>
              </div>
            ))}
