...
Thank you for being part of Online Job.
...
Online Job Team`;
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;font-size:15px;color:#111827;line-height:1.6;max-width:600px;">
      ...
      <p>Thank you for being part of Online Job.</p>
      <p style="color:#125C37;font-weight:700;margin-top:12px;">Online Job Team</p>
    </div>`;
...
export async function sendApprovalEmail({ userEmail, userName, taskName, amount, updatedBalance }) {
  try {
    ...
    await transporter.sendMail({
      from: `"Online Job" <${process.env.SMTP_USER}>`,
      to: String(userEmail),
      subject, text, html,
    });
    return { success: true };
