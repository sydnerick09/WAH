...
// A browser (Chrome, Firefox, the in-app webview, etc.) is bound to a single
// account's email. The binding survives logout, only that account can sign in
// on this browser, and no second account can be created here.
const DEVICE_KEY = 'bh_device_account';

export function getBoundEmail() {
  if (typeof window === 'undefined') return null;
  try {
    // Fall back to the older key name for browsers bound before the rename
    return localStorage.getItem(DEVICE_KEY) || localStorage.getItem('bh_device_registered') || null;
  } catch { return null; }
}

export function bindBrowser(email) {
  if (typeof window === 'undefined' || !email) return;
  try { localStorage.setItem(DEVICE_KEY, String(email).toLowerCase()); } catch (_) {}
}
...
export async function createWithdrawalRequest(userId, { fullName, phone, idNumber, amount }) {
  if (hasSupabase) {
    const { data } = await proxy('createWithdrawal', { userId, fullName, phone, idNumber, amount });
    return data;
  }
...
export async function changeContact(userId, { email, phone }) {
  if (hasSupabase) {
    const r = await proxy('changeContact', { newEmail: email, newPhone: phone });
    if (r.success && r.user) setCurrentUser(r.user);
    return r;
  }
  const users = lsGetUsers();
  ...
  users[idx].premiumPaidAt = null;
  users[idx].premium_paid_at = null;
  lsSave(users);
  const out = norm(users[idx]);
  setCurrentUser(out);
  return { success: true, user: out };
...
// Server-authoritative fee quote (live USD→KES + capped deductions from history).
export async function bulkWithdrawalQuote(paidBefore) {
  if (hasSupabase) return proxy('bulkWithdrawalQuote', { paidBefore: !!paidBefore });
  // Local dev fallback (no live FX / history): full USD 23 at an approximate rate.
  const rate = 129, convertedKes = Math.round(23 * rate);
  return { success: true, eligible: true, balance: 0, feeUsd: 23, rate, rateLive: false,
    convertedKes, recordedFees: 0, eligibleDeductions: 0, perFeeKes: 650, maxDeductions: 2,
    deductionKes: 0, amountDueKes: convertedKes };
}
...
export async function submitBulkWithdrawal(details) {
  ...
  return bulkWithdrawalQuote(details?.paidBefore);
}
